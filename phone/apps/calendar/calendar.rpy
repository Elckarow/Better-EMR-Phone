init -100 python in phone.calendar:
    ...
    def add_calendar(calendar, key=None):
        if renpy.is_init_phase():
            store.config.start_callbacks.append(renpy.partial(add_calendar, calendar, key))
        else:
    ...
    def add_calendar_to_all_characters(calendar):
        if renpy.is_init_phase():
            store.config.start_callbacks.append(renpy.partial(add_calendar_to_all_characters, calendar))
        else:
